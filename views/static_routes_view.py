...
class StaticRoutesViews(ctk.CTkFrame):
    ...
    def build_left_column(self):
        self.add_frame = ctk.CTkFrame(self, **self.card_style)
        self.add_frame.grid(row=0, column=0, sticky="nsew", padx=(20, 10), pady=20)
        self.add_frame.grid_columnconfigure(0, weight=1)

        ctk.CTkLabel(
            self.add_frame, text="Agregar Ruta Estática", font=self.header_font, text_color="#C2185B"
        ).grid(row=0, column=0, sticky="w", padx=20, pady=(20, 15))

        ctk.CTkLabel(
            self.add_frame, text="Destino (red/máscara)", font=self.label_font, text_color="#AD1457"
        ).grid(row=1, column=0, sticky="w", padx=20)
        self.dest_entry = ctk.CTkEntry(self.add_frame, placeholder_text="192.168.1.0/24", font=self.label_font)
        self.dest_entry.grid(row=2, column=0, sticky="ew", padx=20, pady=(2, 15))

        ctk.CTkLabel(
            self.add_frame, text="Gateway (IP)", font=self.label_font, text_color="#AD1457"
        ).grid(row=3, column=0, sticky="w", padx=20)
        self.gateway_entry = ctk.CTkEntry(self.add_frame, placeholder_text="10.0.0.1", font=self.label_font)
        self.gateway_entry.grid(row=4, column=0, sticky="ew", padx=20, pady=(2, 15))

        ctk.CTkLabel(
            self.add_frame, text="Comentario (opcional)", font=self.label_font, text_color="#AD1457"
        ).grid(row=5, column=0, sticky="w", padx=20)
        self.comment_entry = ctk.CTkEntry(self.add_frame, placeholder_text="Ej: Red LAN", font=self.label_font)
    ...
    def build_right_column(self):
        self.list_frame = ctk.CTkFrame(self, **self.card_style)
        self.list_frame.grid(row=0, column=1, sticky="nsew", padx=(10, 20), pady=20)
        self.list_frame.grid_columnconfigure(0, weight=1)
        self.list_frame.grid_rowconfigure(1, weight=1)

        ctk.CTkLabel(
            self.list_frame, text="Rutas Estáticas Actuales", font=self.header_font, text_color="#C2185B"
        ).grid(row=0, column=0, sticky="w", padx=20, pady=(20, 15))
    ...
    def create_route_card(self, route, index):
        ...
        ctk.CTkLabel(
            card, text=dest_text, font=self.small_bold_font, text_color="#880E4F", anchor="w"
        ).grid(row=0, column=1, sticky="w", padx=(0, 15), pady=(10, 2))

        ctk.CTkLabel(
            card, text=gateway_text, font=self.small_font, anchor="w", text_color="#6A1B9A"
